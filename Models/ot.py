...
from utils import ot_nanmean, ot_MAE, ot_RMSE
...
class OTimputer():
    ...
    def fit_transform(self, X, verbose=True, report_interval=500, X_true=None):
        ...
        imps = (self.noise * torch.randn(mask.shape).double() + ot_nanmean(X, 0))[mask.bool()]
        ...
        for i in range(self.niter):
            ...
            if X_true is not None:
                maes[i] = ot_MAE(X_filled, X_true, mask).item()
                rmses[i] = ot_RMSE(X_filled, X_true, mask).item()
        ...
        for i in range(self.max_iter):
            ...
            for l in range(d):
                ...
                for k in range(self.niter):
                    ...
                    #print(self.models[j](X_filled[mask[:, j].bool(), :][:, np.r_[0:j, j+1: d]]))
